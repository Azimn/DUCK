...
from duck import DuckRuntime, MechanisticSnapshot, RecognitionSignal, SubjectAccessFirewall
...
class SpyCognition:
    ...
    def generate(self, moment):
        self.seen = moment
        return InnerCognition("I need a moment to think.")


def test_private_cognition_never_receives_raw_floats():
    spy = SpyCognition()
    ...
    assert spy.seen is result.subjective_moment
...
def test_recognition_uncertainty_is_human_like_not_numeric():
    ...
    assert "0.64" not in recognition.content
    assert "64%" not in recognition.content
...
def test_introspective_cause_can_remain_hidden_from_subject():
    ...
    assert "unresolved argument" not in repr(result.subjective_moment)
...
def test_cause_may_become_accessible_when_the_subject_has_access_to_it():
    ...
    moment = SubjectAccessFirewall().project(snapshot)
    impression = next(i for i in moment.impressions if "uneasy" in i.content.lower())
    assert impression.content == "I feel uneasy about what happened yesterday."
...
def test_inner_speech_is_optional_but_action_can_still_happen():
    ...
    assert any(t.felt_as == "I want to step back." for t in result.subjective_moment.tendencies)


def test_deterministic_inner_voice_uses_first_person_subjective_state():
    runtime = DuckRuntime()
...
def test_subjective_types_have_no_float_fields():
    moment = SubjectAccessFirewall().project(
        MechanisticSnapshot(affect={"fear": 0.55}, relationship={"trust": 0.20}, recognition=RecognitionSignal("Sarah", 0.55))
    )
    assert not _contains_float(moment)
    assert not _contains_float(asdict(moment))
